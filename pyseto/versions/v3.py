...
class V3Local(LocalKey):
    ...
    def to_paserk(self, wrapping_key: Union[bytes, str] = b"") -> str:
        ...
        if not wrapping_key:
            return "k3.local." + base64url_encode(self._key).decode("utf-8")
        bkey = (
            wrapping_key
            if isinstance(wrapping_key, bytes)
            else wrapping_key.encode("utf-8")
        )
        return "k3.local-wrap.pie." + self._encode_pie(bkey, self._key)
    ...
    def _encode_pie(cls, wrapping_key: bytes, ptk: bytes) -> str:
        ...
        t = cls._generate_hash(ak, b"k3.local-wrap.pie." + n + c, 48)
    ...
    def _decode_pie(cls, wrapping_key: bytes, data: str) -> bytes:
        ...
        c = d[80:]
        ak = cls._generate_hash(wrapping_key, b"\x81" + n, 32)
        t2 = cls._generate_hash(ak, b"k3.local-wrap.pie." + n + c, 48)
        if t != t2:
